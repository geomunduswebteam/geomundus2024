...
if($imageFileType != "pdf" && $imageFileType != "doc" && $imageFileType != "zip" ) {
    echo "Sorry, only PDF, DOC and ZIP files are allowed.";
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
		...
		$to2      = 'calia@uji.es';
        $subject = 'Extended Abstract Submission';
        $message = 'Someone upload a Extended Abstract Submission. Check in the ftp.';
...
echo 'alert("document successfully sent")';
echo '</script>';
